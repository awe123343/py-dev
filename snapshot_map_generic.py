...
@dataclass
class HistoryEntry(Generic[VT]):
    """A Plain Old Python Object to store a value at a specific snapshot moment."""
    snap_id: int
    value: VT | None

class SnapshottableMap(Generic[KT, VT]):
    """
    A generic map-like class that supports taking snapshots of its state.
    ...
    def __init__(self):
        ...
        self._key_history_store: dict[KT, list[HistoryEntry[VT]]] = {}
    ...
    def put(self, k: KT, v: VT | None):
        """
        Associates a value with a key, recording the change for the next snapshot.

        Args:
            k: The key (generic type KT).
            v: The value (generic type VT or None for deletion).
        """
        history: list[HistoryEntry[VT]] = self._key_history_store.setdefault(k, [])
    ...
    def delete(self, k: KT):
        ...
        Args:
            k: The key (generic type KT) to delete.
        """
        if k in self._key_history_store:
            self.put(k, None)

    def get(self, k: KT) -> VT:
        """
        ...
        """
        history: list[HistoryEntry[VT]] | None = self._key_history_store.get(k)
        if not history:
            raise KeyError(f"Key '{k}' not found in current map state.")
        
        latest_value: VT | None = history[-1].value
        if latest_value is None:
            raise KeyError(f"Key '{k}' not found (it may have been deleted).")
        return latest_value
            
    def get_from_snapshot(self, k: KT, snap_id: int) -> VT | None:
        """
        ...
        if not (0 <= snap_id < self._next_snap_id):
            valid_range: str = "No snapshots taken yet." if self._next_snap_id == 0 else f"0 to {self._next_snap_id - 1}."
            raise IndexError(f"Invalid snap_id: {snap_id}. {valid_range}")
        
        history: list[HistoryEntry[VT]] | None = self._key_history_store.get(k)
        if not history:
            raise KeyError(f"Key '{k}' never existed, so it's not in snapshot {snap_id}.")
        ...
        if idx == -1:
            logging.error(f"Key '{k}' did not exist in snapshot {snap_id}.")
            # raise KeyError(f"Key '{k}' did not exist in snapshot {snap_id}.")
            return None

        relevant_value: VT | None = history[idx].value
        if relevant_value is None:
            logging.error(f"Key '{k}' did not exist in snapshot {snap_id} (it was deleted).")
            return None

        return relevant_value

if __name__ == "__main__":
    s = SnapshottableMap[str, int]()
    s.put("1", 1)
    s.put("2", 2)
    s.put("3", 3)
...
# Snapshots
# 	•	After the first three puts, you call snapshot(). This records snapshot 0.
# Output: 0
# 	•	After put(1, 2), you call snapshot() again. This records snapshot 1.
# Output: 1
# 	•	After two more puts, you call snapshot() again. This records snapshot 2.
# Output: 2
#
# Getting values
# 	•	s.snapshot_get(1, 0)
# → Value for key 1 at snapshot 0 is 1 (from first put).
# 	•	s.snapshot_get(1, 1)
# → Value for key 1 at snapshot 1 is 2 (updated after second put and new snapshot).
# 	•	s.snapshot_get(1, 2)
# → Value for key 1 at snapshot 2 is 2 (not updated since previous snapshot).
# 	•	s.snapshot_get(4, 0)
# → Key 4 did not exist at snapshot 0, so None.
# 	•	s.snapshot_get(4, 1)
# → Key 4 did not exist at snapshot 1, so None.
# 	•	s.snapshot_get(4, 2)
# → Key 4 was set to 4 before snapshot 2, so 4.