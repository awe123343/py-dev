...
class SnapshottableMap[KT, VT]:
    ...
    def put(self, k: KT, v: VT | None) -> None:
        """
    ...
    def get_from_snapshot(self, k: KT, snap_id: int) -> VT | None:
        ...
        if relevant_value is None:
            logger.error(
                "Key '%s' did not exist in snapshot %s (it was deleted).",
                k,
                snap_id,
            )
            return None
